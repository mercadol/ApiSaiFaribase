...
describe('BaseController', () => {
  ...
  beforeEach(() => {
    // Se crea un servicio simulado con métodos jest.fn()
    service = {
      getAll: jest.fn(),
      getById: jest.fn(),
      create: jest.fn(),
    ...
    // Reseteamos el mock de firebase.db.collection para que por defecto retorne que el doc existe.
    firebase.db.collection.mockReturnValue({
      doc: jest.fn().mockReturnValue({
        get: jest.fn(() =>
          Promise.resolve({ exists: true, id: 'dummyDocId' })
        ),
      }),
    });
  });
  ...
  describe('getAll', () => {
    ...
    it('debe utilizar el documento obtenido de firebase cuando se pasa startAfter y este existe', async () => {
      // Configuramos la respuesta del servicio
      const items = [{ id: 3 }];
      const lastDoc = { id: 'lastDocId2' };
      service.getAll.mockResolvedValue({ items, lastDoc });

      // Simulamos query con startAfter y pageSize
      req.query = { startAfter: 'doc123', pageSize: '5' };

      // firebase.db.collection debe ser llamado con el id de la colección del servicio.
      // Nuestro mock por defecto retorna un documento con exists:true.
      await controller.getAll(req, res);

      // Verificamos que se haya llamado a service.getAll pasando el documento simulado
  ...
  describe('create', () => {
    ...
    it('debe retornar error 400 si la validación de creación falla', async () => {
      ...
      controller.validateCreateData = jest.fn().mockReturnValue('Error de validación');
      req.body = { name: '' };
  ...
  describe('update', () => {
    ...
    it('debe retornar error 400 si la validación de actualización falla', async () => {
      ...
      controller.validateUpdateData = jest.fn().mockReturnValue('Actualización inválida');
      req.params = { id: 'entity123' };
      req.body = { name: '' };
      ...
      expect(res.status).toHaveBeenCalledWith(400);
      expect(res.json).toHaveBeenCalledWith({ error: 'Actualización inválida' });
    });

...
const memberGroupService = {
  ...
  getGroupsOfMember: async (memberId) => {
    try {
      ...
      if (querySnapshot.empty) {
        throw new Error(`No se encontraron grupos para el miembro con ID: ${memberId}`);
      }
      ...
      throw new Error('Error al obtener los grupos. Por favor, inténtalo más tarde.');
    }
  },
  ...
  getMembersOfGroup: async (groupId) => {
    try {
      ...
      if (querySnapshot.empty) {
        throw new Error(`No se encontraron miembros para el grupo con ID: ${groupId}`);
      }
      ...
      const miembros = await Promise.all(
        miembroIDs.map(async (memberId) => {
          ...
          return miembroDoc.exists ? { id: memberId, ...miembroDoc.data() } : null;
        })
      );
      ...
      throw new Error('Error al obtener los miembros. Por favor, inténtalo más tarde.');
    }
  },
  ...
  addMemberToGroup: async (memberId, groupId) => {
    try {
      if (!memberId || !groupId) throw new Error('El ID del miembro y del grupo son obligatorios.');

      const docId = `${memberId}_${groupId}`;
      await db.collection('MiembroGrupo').doc(docId).set({ memberId, groupId, fecha: new Date().toISOString() });
      return { message: 'Relación creada con éxito.', id: docId };
    } catch (error) {
      console.error('Error al añadir miembro al grupo:', error.message);
      // 🔹 Lanzar errores específicos según la causa
      if (error.message === "El ID del miembro y del grupo son obligatorios.") {
        throw error;
      } else {
        throw new Error("Error al crear la relación. Por favor, inténtalo más tarde.");
      }
    }
  },
  ...
  removeMemberFromGroup: async (memberId, groupId) => {
    try {
      const querySnapshot = await db.collection("MiembroGrupo")
        .where("memberId", "==", memberId)
        .where("groupId", "==", groupId)
        .get();
    
      if (querySnapshot.empty) {
        // ✅ Lanzamos el error correctamente ANTES de entrar al catch
        return Promise.reject(new Error(`No se encontró la relación miembro-grupo para memberId: ${memberId} y groupId: ${groupId}`));
      }
      ...
      console.error("Error al eliminar la relación miembro-grupo:", error.message);
      ...
      throw new Error("Error al eliminar la relación. Por favor, inténtalo más tarde.");
    }
